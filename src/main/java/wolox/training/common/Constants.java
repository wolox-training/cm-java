package wolox.training.common;

public class Constants {

    public static final String MESSAGE_ERROR_NOT_FOUND_USER = "Not found user";
    public static final String MESSAGE_ERROR_NOT_FOUND_BOOK = "Not found book";
    public static final String URL_BOOKS = "/books";
    public static final String URL_USERS = "/users";
    public static final String URL_EXTERNAL_API = "api/books?bibkeys=ISBN:";
    public static final String URL_EXTERNAL_API_PARAM = "&format=json&jscmd=data";
    public static final String KEY_ISBN = "ISBN:";
    public static final String SORT_BOOK = "title";
    public static final String SORT_USER = "name";
    public static final String URL_LOCAL = "http://localhost:8080/api/books";
    public static final String PROFILE_DEV = "dev";
    public static final String PROFILE_TEST = "test";


}
